const updateDaySpread = (direction) => {
    ...
    xhttp.onreadystatechange = function () {
        if (this.readyState == 4 && this.status == 200) {
            ...
            let tags = '';
            tags += `<div style="margin:8px;"><span class="d-inline-block w-50">${maxDate}:</span>$${btcMax.toFixed(2)}</div>`;
            tags += `<div style="margin:8px;"><span class="d-inline-block w-50">${minDate}:</span>$${btcMin.toFixed(2)}</div><hr />`;
    ...
    if (direction === 'increase') {
        document.getElementById('spread').innerHTML = `${
            Number(document.getElementById('spread').innerHTML) + 1
            }`;
    ...
    past.setDate(past.getDate() - (Number(document.getElementById('spread').innerHTML) < 1 ? 1 : Number(document.getElementById('spread').innerHTML)));
    xhttp.open(
        "GET",
        (
